...
class BeautyService:
    ...
    def export_to_excel(self, customer_name, customer_phone):
        try:
        ...
        sheet.append([customer_name, customer_phone, self.name, self.price])
...
class BeautySalon:
    ...
    def record_appointment(self, customer, service):
        ...
        customer.earn_loyalty_points(service.price*0.1)  # Например, начисление баллов за запись на услугу
        print(f"{customer.name} получил {service.price*0.1} баллов лояльности.")
    ...
    while True:
        ...
        if choice == '1':
            ...
            if customer:
                ...
                service.export_to_excel(customer.name, customer.phone)
            else:
